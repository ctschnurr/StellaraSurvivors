...
func fire_raycast(direction: Vector2):
	...
	if !raycast_output.is_empty():
		...
		var new_direction = asteroid_direction.reflect(collision_normal).normalized()
...
#func _draw():
	#var look_left = asteroid_direction.rotated(-0.8)
	#var look_right = asteroid_direction.rotated(0.8)
		
	#draw_line(Vector2.ZERO, asteroid_direction * 20, Color.GREEN)
	#draw_line(Vector2.ZERO, look_left * 20, Color.GREEN)
	#draw_line(Vector2.ZERO, look_right * 20, Color.GREEN)
