...
var explode_strength = 1

func _ready():
	$Area2D.area_entered.connect(on_area_entered)
	pulse.shape = shape_resource.duplicate()
	explode(explode_strength)
...
func explode(strength: int):
	...
	tween.tween_property(self, "scale", Vector2(strength + 5, strength + 5), 0.375).set_ease(Tween.EASE_IN).set_trans(Tween.TRANS_SINE)
