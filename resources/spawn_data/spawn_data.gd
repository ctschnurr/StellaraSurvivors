...
@export_group("Spawn Amount Settings:")
@export var spawn_per_wave: int = 1

enum Amount_behavior{STATIC, DYNAMIC}
@export var amount_behavior: Amount_behavior = Amount_behavior.STATIC

@export_subgroup("If 'Dynamic':")
enum Change_by{WAVE, MINUTES, KILL_COUNT}
@export var change_by: Change_by = Change_by.MINUTES
@export var change_interval: int = 1
@export var change_amount: int = 1
...
@export var total_time_in_minutes: float
...
@export var behavior_variation: Wave_behavior_variation = Wave_behavior_variation.STEADY
